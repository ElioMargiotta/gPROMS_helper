...
def build_comparisons(d1: Dict[str, float], d2: Dict[str, float]) -> List[Tuple[str, Optional[float], Optional[float], Optional[float], Optional[float], str]]:
    """Return list of tuples: (name, v1, v2, abs_diff, rel_diff, note)"""
    ...
    for name, (v1, v2, note) in pre_map.items():
        if v1 is None or v2 is None:
            ...
            rel = abs_diff / denom
        comps.append((name, v1, v2, abs_diff, rel, note))
    ...
    for k in list(keys):
        ...
        if v1 is None or v2 is None:
            ...
            denom = max(abs(v1), abs(v2), 1e-12)
            rel = abs_diff / denom
        comps.append((k, v1, v2, abs_diff, rel, note))

    # Sort by absolute difference descending (missing values go last)
    comps.sort(key=lambda t: (abs(t[3]) if t[3] is not None else -1), reverse=True)
    return comps
...
def write_csv(out_path: str, comps: List[Tuple[str, Optional[float], Optional[float], Optional[float], Optional[float], str]]):
    with open(out_path, "w", encoding="utf-8", newline="") as fh:
        ...
        writer.writerow(["variable", "value_file1", "value_file2", "abs_diff", "rel_diff", "note"])
        for name, v1, v2, abs_diff, rel, note in comps:
            writer.writerow([
                name,
                "" if v1 is None else f"{v1:.16e}",
                "" if v2 is None else f"{v2:.16e}",
                "" if abs_diff is None else f"{abs_diff:.16e}",
                "" if rel is None else f"{rel:.6e}",
                note,
            ])
...
def plot_differences(plot_path: str, comps: List[Tuple[str, Optional[float], Optional[float], Optional[float], Optional[float], str]], top_n: int = 40):
    # Choose entries where we have both values and abs_diff is not None
    numeric = [c for c in comps if c[3] is not None]
    ...
    diffs = [c[3] for c in numeric]

    plt.figure(figsize=(max(8, len(names) * 0.25), 6))
    bars = plt.bar(range(len(names)), diffs, color="tab:blue")
    plt.axhline(0, color="black", linewidth=0.6)
    plt.xticks(range(len(names)), names, rotation=90)
    plt.ylabel("Absolute difference (file1 - file2)")
    plt.title("Top differences between flowsheet results (top %d)" % len(names))
    plt.tight_layout()
...
def main():
    ...
    # Summaries
    total = len(comps)
    overlap = len([c for c in comps if c[3] is not None])
